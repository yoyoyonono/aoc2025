...
fn main() {
    ...
    let mut last_connection = distances[0];
    while connections.len() < 1 || circuits.len() > 1 || circuits.iter().nth(0).unwrap().len() != input.len() {
        let try_closest = find_closest(&input, &connections, &distances);
        last_connection = try_closest.into();
        println!("{:?}", try_closest);
    ...
    println!("{:?}", product);
    println!("{:#?}", last_connection);
    println!("{}", last_connection.a[0] * last_connection.b[0])
